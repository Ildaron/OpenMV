...
int main(void)
{
  ...
  MX_DCMI_Init();
   ...
   // HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, GPIO_PIN_SET);
    ...
    uint32_t AddressFrameBuffer[0xB160];   //B160
    uint32_t DataSizeImageSensor = 0x25800;

 //DataSizeImageSensor = (uint32_t)((IMG_ROWS * IMG_COLUMNS * 2) / 4))
  //(640*480*2)/4 =
    ...
    void write_register(uint8_t register_pointer, uint16_t register_value)
    {
        ...
        data[2] = register_value<<8;       // LSB byte of 16bit data

        HAL_I2C_Master_Transmit(&hi2c4, 0x42, data, 3, 100);  // data is the start pointer of our array
    }

     uint8_t adress_write = 0x12;
     uint16_t  status_write = 0x06;
     uint8_t  status = 0x1D;
  ...
  while (1)
  {
	...
	//  write_register(0x12, 0x0006);
	  write_register(0x12, 0x05);
  //  status = HAL_I2C_Mem_Write(&hi2c4, 0xA0, (uint8_t)adress_write, I2C_MEMADD_SIZE_8BIT, (uint16_t*)(&status_write), 2, 100);

	  HAL_Delay (60);

	  HAL_I2C_Master_Transmit(&hi2c4, 0x43, &adress_write, 1, 100);
	  HAL_I2C_Master_Receive(&hi2c4, 0x43, &status_readed1, 2, 100);
